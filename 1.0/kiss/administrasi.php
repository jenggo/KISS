<?php
				...
				elseif ($halaman_login == 'kategori') {
					...
					elseif (isset($_POST['hapus']) AND !empty($_POST['id_kategori'])) {
						$num = 0;
					...
					if (isset($_POST['edit']) AND !empty($_POST['id_kategori'])) {
						if (count($_POST['id_kategori']) > 1)
							...
							<table class="clear">
								...
								<tbody>';
									...
									foreach ($list_kategori as $lk) {
										...
										$konten .= '
										<tr'.$class.'>
											<td><input type="checkbox" name="pilihan[]" value="'.$lk['id'].'" /></td>
											<td>'.$lk['nama'].'</td>
											<td>'.$lk['deskripsi'].'</td>
										</tr>';
										$num++;
				...
				elseif ($halaman_login == 'konten' OR $halaman_login == 'pages') {
					...
					# Bila menampilkan konten ambil dahulu data kategori untuk dipakai kemudian
					if ($tipe == 0) {
						...
						<p>
							...
							foreach ($kategori as $k)
								$form_kategori .= '<option value="'.$k['id'].'">'.$k['nama'].'</option>';
					...
					$konten .= '
						<form method="post" action="">
							...
							<p>
								...
								<textarea name="isi" id="isi" rows="19" cols="30"></textarea>
							</p>
							<p class="no-border">
						...
						<form method="post" action="">
							<table class="clear">
								...
								foreach ($list_konten_atau_pages as $lkp) {
									...
									$konten .= '
										<tr'.$class.'>
											<td><input type="checkbox" name="pilihan[]" value="'.$lkp['id'].'" /></td>
											<td>'.$tanggalnya.'</td>
											<td>'.$kategori.'</td>
											<td>'.$lkp['judul'].'</td>
										</tr>
							...
							<p class="no-border">
								<input type="submit" name="hapus" value="Hapus" class="button" /> '.$tombol_ubah.'
							</p>
						</form>';
					}
				}
